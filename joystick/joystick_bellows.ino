...
void setup() {
    Serial.begin(115200);
    joystick.begin();
    multicore_launch_core1([](){ joystick.runTask(); });
...
void loop() {
    ...
    int x = joystick.state.x;
    int y = joystick.state.y;
    bool btn = joystick.state.button;

    // CC X → CC2
    if(x != lastX){
        hv_sendMessageToReceiverFF(&pd_prog, HV_CTLIN_HASH, 0.0f, 2, map(x,0,4095,0,127));
        lastX = x;
    }

    // CC Y → CC1
    if(y != lastY){
        hv_sendMessageToReceiverFF(&pd_prog, HV_CTLIN_HASH, 0.0f, 1, map(y,0,4095,0,127));
        lastY = y;
    }

    // Button → Note 60
    if(btn != lastBtn){
        if(btn) hv_sendMessageToReceiverFF(&pd_prog, HV_NOTEIN_HASH, 0.0f, 60, 100);
        else    hv_sendMessageToReceiverFF(&pd_prog, HV_NOTEIN_HASH, 0.0f, 60, 0);
        lastBtn = btn;
    }
